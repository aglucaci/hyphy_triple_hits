...
def read_json_fullfilter(filename): #Pvalue and ER Thresholdings for sensitive sites
    global codon_change, file_passed_threshold, num_ER_thresholded_sites, TH_Threshold, pvalue_threshold
    this_row = []
    with open(filename, "r") as fh:
        ...
        Threshold_TH = TH_Threshold * np.mean(EvidenceRatio_TH)
        ...
        for i, item in enumerate(EvidenceRatio_TH):
            if item in Filtered_Threshold_TH:
                try:
                    ...
                    #print(file_passed_threshold, filename, Site_subs[str(i)])
                    ...
                    pass
        mode = "a+"
        writetofile("p" + str(pvalue_threshold).replace(".", "-") + "_Threshold_" + str(TH_Threshold) + "x_.txt", filename.split("/")[-1].replace(".FITTER.json", ""), mode)
...
def read_json_p_filter(filename): #Pvalue and ER Thresholdings for sensitive sites
    ...
    with open(filename, "r") as fh:
        ...
        writetofile("p" + str(pvalue_threshold).replace(".", "-") + ".txt", filename.split("/")[-1].replace(".FITTER.json", ""), "a+")
        file_passed_threshold += 1
    fh.close()
...
def read_json_nofilter(filename):
    ...
    fh.close()
    this_row.append(Site_subs)
    writetofile("allfiles.txt", filename.split("/")[-1].replace(".FITTER.json", ""), "a+")
    if len(this_row) == 0: return
    subprocessing_thisrow(this_row)
...
codon_change, file_passed_threshold, num_ER_thresholded_sites = {}, 0 , 0
print("\n", "() p < 0.005, TH Threshold (3x)") 
main_sub("", "FULLFILTER")

TH_Threshold = 10
codon_change, file_passed_threshold, num_ER_thresholded_sites = {}, 0 , 0
print("\n", "() p < 0.005, TH Threshold (10x)") #REMEMBER TO CHANGE THE THRESHOLD IN THE FUNCTION
main_sub("", "FULLFILTER")
