...
//KeywordArgument ("double-split", "Use a separate rate parameter for adjacent and non-adajacent double hits", "No");
...
fitter.analysis_description = {terms.io.info : "Examine whether or not a codon alignment is better fit by models which permit multiple instantaneous substitutions",
                           ...
                           terms.io.authors : "SPITS - Sergei L Kosakovsky Pond, Sadie Wisotsky and Alexander G. Lucaci",
...
utility.Extend (fitter.one_hit_results[terms.global],
                {
                    terms.parameters.multiple_hit_rate1 : { utility.getGlobalValue ("terms.fit.MLE") : 0.05, terms.fix : FALSE},
                    terms.parameters.multiple_hit_rate2 : { utility.getGlobalValue ("terms.fit.MLE") : 0.05, terms.fix : FALSE}
...
fprintf(stdout, io.FormatTableRow(
    {
     ...
     Format (((fitter.two_hit_results[terms.global])[terms.parameters.multiple_hit_rate1])[terms.fit.MLE],8,4) ,
     Format (((fitter.two_hit_results[terms.global])[terms.parameters.multiple_hit_rate2])[terms.fit.MLE],8,4) ,
...
fprintf(stdout, io.FormatTableRow(
    {
     ...
     Format (((fitter.three_hit_results[terms.global])[terms.parameters.multiple_hit_rate1])[terms.fit.MLE],8,4),
     Format (((fitter.three_hit_results[terms.global])[terms.parameters.multiple_hit_rate2])[terms.fit.MLE],8,4),
...
//Printing our likelihood_function
Export(lf, ^(fitter.three_hit_results[terms.likelihood_function]));
fprintf("/Users/Sadie/hyphy-analyses/FitMultiModel/likelihood.txt", lf, "\n");
