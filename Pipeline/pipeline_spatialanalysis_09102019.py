...
def plotly_subplot_mod(data_x, data_y_TH, data_y_DH, output_title, output):
    ...
    for item in range(num_items): # just the count.
        ...
        fig.add_trace(go.Scatter(x=data_x[item], y=data_y_DH[item], opacity=0.65, fillcolor='black'),
                  row=row_count, col=col_count)
        
        """ THIS IS TO ADD DH TO THE OUTPUT PLOT """
        """
    ...
    plot(fig, filename=OUTPUT_DIR+"plots_spatialanalysis_v2.html") #auto_open=False for sanity.
