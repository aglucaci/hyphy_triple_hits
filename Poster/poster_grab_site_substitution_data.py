...
def read_json(filename):
    ...
    #print(filename, this_row[0])
    #print(this_row, "\n", type(this_row), len(this_row))
    #print()
    #print(type(this_row[0]))
    #print(this_row[1])
    
    for k in this_row[0]:
        #print(k, this_row[0][k])
        #print()
        #so Sites (#) are the keys
        #they print out dicts
        
        #print(k, this_row[0][k])
        #print(type(this_row[0][k]))
        
        for keys in this_row[0][k]:
            #print(keys, this_row[0][k][keys]) #this is the FROM codon
            
            #print("From this codon:", keys)
            
            for to_codon in this_row[0][k][keys]:
                #print(this_row[0][k][keys], to_codon)
                #print(to_codon, len(this_row[0][k][keys][to_codon])) #this is the TO codon
                #print(to_codon)
                
                #counts = 
                from_codon = keys
                
                #print(type(diff_count(from_codon, to_codon)))
...
print("loop over files", len(files))
...
#Serine to Serine contribution
print("Counting serine contributions")
SERINE_CODONS = ["TCT", "TCC", "TCA", "TCG", "AGT", "AGC"]

codon_table = {}

codon_table["SER"] = ["TCT", "TCC", "TCA", "TCG", "AGT", "AGC"] #SERINE IS THE ONLY SYNONYMOUS TH CHANGE
codon_table["PHE"] = ["TTT", "TTC"]
codon_table["LEU"] = ["TTA", "TTG", "CTT", "CTC", "CTA", "CTG"]
codon_table["ILE"] = ["ATT", "ATC", "ATA"]
codon_table["VAL"] = ["GTT", "GTC", "GTA", "GTG"]
codon_table["PRO"] = ["CCT", "CCC", "CCA", "CCG"]
codon_table["THR"] = ["ACT", "ACC", "ACA", "ACG"]
codon_table["ALA"] = ["GCT", "GCC", "GCA", "GCG"]
codon_table["TYR"] = ["TAT", "TAC"]
codon_table["HIS"] = ["CAT", "CAC"]
codon_table["GLN"] = ["CAA", "CAG"]
codon_table["ASN"] = ["AAT", "AAC"]
codon_table["LYS"] = ["AAA", "AAG"]
codon_table["ASP"] = ["GAT", "GAC"]
codon_table["GLU"] = ["GAA", "GAG"]
codon_table["CYS"] = ["TGT", "TGC"]
codon_table["ARG"] = ["CGT", "CGC", "CGA", "CGG"]
...
#print(codon_change)
for k, v in codon_change.items():
    for key, value in codon_change[k].items():
        total_exchanges += codon_change[k][key]
        if k in SERINE_CODONS and key in SERINE_CODONS: #S2S exchange
            Serine_sum += codon_change[k][key]
            
print("Total S2S:", Serine_sum)
print("Total exchanes:", total_exchanges)

codon_table_count = {}

codon_table_count["SER"] = 0
codon_table_count["PHE"] = 0
codon_table_count["LEU"] = 0
codon_table_count["ILE"] = 0
codon_table_count["VAL"] = 0
codon_table_count["PRO"] = 0
codon_table_count["THR"] = 0
codon_table_count["ALA"] = 0
codon_table_count["TYR"] = 0
codon_table_count["HIS"] = 0
codon_table_count["GLN"] = 0
codon_table_count["ASN"] = 0
codon_table_count["LYS"] = 0
codon_table_count["ASP"] = 0
codon_table_count["GLU"] = 0
codon_table_count["CYS"] = 0
codon_table_count["ARG"] = 0
codon_table_count["GLY"] = 0


#Break down by synonymous exchanges.
for key1, value1 in codon_change.items():
    #key1 is codon from
    
    for key2, value2 in codon_change[key1].items():
        #key2 is codon to
        
        for key3, value3 in codon_table.items():
            #key3 is AA
            
            if key1 in codon_table[key3] and key2 in codon_table[key3]: #synonymous change
                #add it to the AA count.
                codon_table_count[key3] += codon_change[key1][key2]



for k, v in codon_table_count.items():
    x = round((codon_table_count[k] / total_exchanges) * 100, 2)
    print("AA:", k, codon_table_count[k]," fraction of total:", str(x) + "%")


#AA1 to AA2 excjamges
#codon_table_AA["SER"] = [ ["TO_CODON", count]. "TO_CODON", count]]
codon_table_AA = {}
codon_table_AA["SER"] = []
codon_table_AA["PHE"] = []
codon_table_AA["LEU"] = []
...
print("Examining AA to AA exchanges")
